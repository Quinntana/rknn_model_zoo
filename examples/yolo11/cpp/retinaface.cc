...
static int post_process_retinaface(retina_app_context_t *app_ctx, image_buffer_t *src_img, rknn_output outputs[], retinaface_result *result, letterbox_t *letter_box) {
...
int init_retinaface_model(const char *model_path, retina_app_context_t *app_ctx, uint32_t init_flags) {
    int ret;
    int model_len = 0;
    char *model;
    rknn_context ctx = 0;
    ...
    app_ctx->preprocess_buf_size = app_ctx->model_width * app_ctx->model_height * app_ctx->model_channel;
    app_ctx->preprocess_buf = (unsigned char *)malloc(app_ctx->preprocess_buf_size);
    if (app_ctx->preprocess_buf == NULL) {
        printf("preprocess buffer allocation fail!\n");
        return -1;
    }
...
int inference_retinaface_model(retina_app_context_t *app_ctx, image_buffer_t *src_img, retinaface_result *out_result) {
    int ret;
    image_buffer_t img;
    ...
    img.virt_addr = app_ctx->preprocess_buf;

    if (img.virt_addr == NULL) {
        printf("preprocess buffer is null! size:%d\n", img.size);
        return -1;
    }
